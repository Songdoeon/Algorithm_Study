...
class Move{
    ...
    public Move(int rx, int ry, int bx, int by, int cnt, boolean redExit,  boolean blueExit, String dir) {
        this.rx = rx;
        this.ry = ry;
        this.bx = bx;
        this.by = by;
...
public class Main {
    ...
    static Move moving(Move move, int dir) {
        ...
        return new Move(rX, rY, bX, bY, move.cnt + 1,move.redExit, move.blueExit, move.dir);
...
//		int idx = 0;
        while(!queue.isEmpty()) {
            ...
            if(m.blueExit) continue;
            for(int i = 0; i < 4; i++) {
                Move next = moving(new Move(m.rx, m.ry, m.bx, m.by, m.cnt, m.redExit, m.blueExit, m.dir), i);
                next.dir += dirs[i];
